...
import { loginUser, resendVerificationEmail } from "../../api/auth.api";
...
export default function Login() {
  ...
  const handleResendVerification = async () => {
    ...
    setResendStatus("");
    try {
      const res = await resendVerificationEmail({ email: form.email });
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md bg-white/80 backdrop-blur-sm rounded-xl2 shadow-soft p-10 space-y-6">
        ...
        {error && (
          <div className="bg-secondary-100 text-secondary-600 text-sm font-body rounded-lg px-4 py-3 space-y-2">
            <p>{error}</p>
            {isUnverified && (
              <button
                type="button"
                ...
                disabled={resending}
                className="text-xs font-semibold text-primary-700 underline hover:text-primary-900 block"
              >
                {resending ? "Sending verification email..." : "Didn't receive email? Click to resend"}
              </button>
            )}
          </div>
        )}
        ...
        {resendStatus && (
          <div className="bg-green-50 text-green-700 text-sm font-body rounded-lg px-4 py-3">
            {resendStatus}
          </div>
        )}
